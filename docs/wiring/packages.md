...
## 2. Settings — package unit prices (money-gated + feature-gated)
- **Category:** a new **Packages** pane in the shared settings modal
  (`shared/board-settings.js` `renderPackagesPane`), visible when
  **`canEditShopMoney && feature_packages`** — i.e. the same money gate as RO &
  Pricing (owner/GM), AND only when the switch is on. Manager (GM board) can edit
  it; advisor cannot.
- **Table:** `public.package_units` — `unit_code` (the dropdown label, e.g.
  `6L80`), `set_price` (customer price), `default_rr_hours` (nullable tech-pay
  default), `active` (default true), timestamps. Anon-full-access RLS + realtime,
  mirroring every CrisData settings list.
- **CRUD:** add / edit / **hard-delete** rows (`addPackageUnit` / `savePackageUnit`
  / `deletePackageUnit`). Deleting a unit that a job already used does **not**
  rewrite that job — see §4 (resolve-and-store + `on delete set null`).
...
- **The row (package line):** in `renderLines()` a `line_type==='package'` row is
  ...
  - **Description cell → a unit dropdown** (`data-field="package_unit_id"`) of the
    active units (plus the stored one if it was since deleted, so the name still
    shows).
...
- **Settings:** `shared/board-settings.js` — `FEATURE_FLAGS` `packages` entry
  (:88), `loadPackageUnits`/`getPackageUnits` (:220/:233), Packages category
  (:480), `renderPackagesPane` + `addPackageUnit`/`savePackageUnit`/
  `deletePackageUnit` (:701+). Exposed via `BoardSettings.getPackageUnits`.
- **RO builder:** `advisor-board.html` — `packagesFeatureOn`/`packageUnits`
  (:3645/:3647), `lineTypeOptions` (:3664), package row in `renderLines` (:4907),
  resolve-and-store in `onFieldChange` (:5170), print fold-in (:5611/:5620).
