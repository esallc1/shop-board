...
> Last updated: 2026-09-25 — **§3 added: ONE shared player** (`shared/recording-view.js`, Inbox slice 2b) for all four places on the advisor board; **LIVE on prod** since `103a645`; the in-place built-in player (§3) **LIVE on prod** since `9dce1d4`.
...
## Session change log
- 2026-09-25 — **in-place built-in player shipped to prod** (Cris's OK, no test look — test has no audio files): fast-forward `0a9539d..6c36e2c` (code = `9dce1d4`; `6c36e2c` = docs-only on top). www / board. / apex `/api/version` = `6c36e2c` (~30 s); `advisor-board.html`, `shared/recording-view.js` (+test) byte-identical to `9dce1d4`, the doc to `6c36e2c`, on all three; CLAUDE.md 404. Prod read-only (not signed in, nothing written, no RO opened): page loads, tray folded, `recording-view.js` 200, `closeOpenPlayer` / `cdRecordingStop` present, the hidden per-place `<audio>` gone, no console errors. Real playback with the in-place player: Cris to test on prod.
- 2026-09-25 — the in-place player driven on test.* at `9dce1d4`: files byte-identical, prod untouched (`0a9539d`). The pane's CrisData sign-in had lapsed, so the page's links reader was replaced in page memory by a stand-in answering every call "ready, 20 s, a generated tone" (nothing sent, nothing written). Real clicks / drags: Call log Aug 12 ▶ → built-in player in place of the button, playing; bar dragged 7.8 s → 17.4 s and back; ⏸ at 3.9 s → ▶ resumed 3.9 → 6.0 → ⏸ held; a second row's ▶ → the first collapsed to "▶ Play (0:20)", one player on the page; that returned button reopened it; closing the Call log → paused + collapsed. Customer record ▶ → player in place; sidebar To-Do → paused + collapsed. No errors. RO Call History not driven this round (same player; unit + static tests).
