...
typed); a dragged chip snaps back. Step (a)3 (2026-09-25, **LIVE on prod** `b9d349c`): the Call Log / customer
...
policies are still open until the lockdown (step (b)): `migrations/20260926_calls_lockdown_{SANDBOX,PROD}.sql`
— written, **NOT RUN**; Cris runs them by hand, sandbox first, once the watch period (no browser
writes to `/rest/v1/calls` in the Supabase API logs, no unexplained `api/calls` 401s) is clear.
