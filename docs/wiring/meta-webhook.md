...
## Session change log
- **2026-09-23** — §12 proven on test.* (`fe74a1c`, SANDBOX migration applied by Cris, 10/10): sim `autoreply` — A closed 5:54am → one dry-run auto row (582 chars, auto=true), A2 same stretch → none; B Business Suite reply then inbound → none; C open 4:49pm → none; D "(239) 887-8557" → `detected_phone` 2398878557 + tray "Attach to Snooks Services". `autoecho` (metadata crisdata:auto, same mid) → still 3 rows, auto stays true, 0 staff rows. `closedtoday` (set in Settings → Facebook, 4:50pm) → auto-reply; switch turned back OFF (`shop_closed_on` null). Auto-replied threads stay waiting in the tray. 16 served files byte-identical.
