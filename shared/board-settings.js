...
window.BoardSettings = (function () {
  ...
  function renderRoPricingPane(content) {
    ...
    content.innerHTML =
      ...
      // Rebuild Book Hours — the shop-set standard-hours lookup the RO builder
      // draws from for flat-priced rebuilds (money/pricing config → Owner/GM).
      (canEditShopMoney ? `<div class="stgfeat-section" id="stgRebuildWrap" style="margin-top:14px">
        <div style="font-weight:700;margin-bottom:2px">Rebuild Book Hours</div>
        <div style="font-size:0.75rem;color:var(--muted);margin-bottom:10px">Standard flagged book hours per rebuild, keyed on transmission code. The RO builder copies the value onto a job when a rebuild type is picked — editing here never changes jobs already saved.</div>
        <div id="stgRebuildBody" style="font-size:0.8rem;color:var(--muted)">Loading…</div>
      </div>` : '');

    content.querySelector('#stgfeatShopSaveBtn').addEventListener('click', saveShopSettings);
    if (canEditShopMoney) renderRebuildHoursSection();
  }

  // ── RO & Pricing extra: Rebuild Book Hours (shop-set standard-hours table) ─
  // Small CRUD over rebuild_book_hours (Owner/GM). RESOLVE-AND-STORE means the
  // RO copies a row's hours at capture, so editing/deactivating here never
  // rewrites past jobs. transmission_code is the unique lookup key.
  async function renderRebuildHoursSection() {
    const body = modalEl && modalEl.querySelector('#stgRebuildBody');
    if (!body) return;
    let rows = [];
    try {
      const { data, error } = await db.from('rebuild_book_hours').select('*').order('transmission_code');
      if (error) throw error;
      rows = data || [];
    } catch (err) {
      body.innerHTML = '<div style="color:var(--muted)">Run the book-hours migration (creates rebuild_book_hours) to manage standard rebuild hours here.</div>';
      return;
    }
    const rowsHtml = rows.length ? rows.map(r => `
      <tr data-id="${esc(String(r.id))}" style="border-top:1px solid var(--border)">
        <td style="padding:6px 6px;font-weight:600">${esc(r.transmission_code)}</td>
        <td style="padding:6px 6px"><input type="text" data-f="label" value="${esc(r.label || '')}" placeholder="(optional)" style="width:100%"></td>
        <td style="padding:6px 6px"><input type="number" data-f="book_hours" value="${r.book_hours != null ? esc(String(r.book_hours)) : ''}" min="0" step="0.5" style="width:66px"></td>
        <td style="padding:6px 6px;text-align:center"><input type="checkbox" data-f="active"${r.active ? ' checked' : ''}></td>
      ...
      </tr>`).join('') : '<tr><td colspan="5" style="padding:8px 6px;color:var(--muted)">No rebuild types yet — add one below.</td></tr>';
    body.innerHTML = `
      <table style="width:100%;border-collapse:collapse;font-size:0.8rem">
        <thead><tr style="text-align:left;color:var(--muted);font-size:0.72rem">
          <th style="padding:2px 6px">Code</th><th style="padding:2px 6px">Label</th>
          <th style="padding:2px 6px">Hours</th><th style="padding:2px 6px;text-align:center">Active</th><th style="padding:2px 6px"></th>
        </tr></thead>
        <tbody>${rowsHtml}</tbody>
        <tfoot><tr style="border-top:2px solid var(--border)">
          <td style="padding:6px 6px"><input type="text" id="stgRbNewCode" placeholder="e.g. 6L80" style="width:100%"></td>
          <td style="padding:6px 6px"><input type="text" id="stgRbNewLabel" placeholder="(optional)" style="width:100%"></td>
          <td style="padding:6px 6px"><input type="number" id="stgRbNewHours" min="0" step="0.5" placeholder="hrs" style="width:66px"></td>
          <td></td>
          <td style="padding:6px 6px"><button type="button" class="stgfeat-btn" id="stgRbAddBtn" style="padding:3px 8px;font-size:0.72rem">Add</button></td>
        </tr></tfoot>
      </table>
      <div class="stgfeat-error" id="stgRbError" style="margin-top:6px"></div>`;
    body.querySelector('#stgRbAddBtn').addEventListener('click', addRebuildRow);
    body.querySelectorAll('tr[data-id]').forEach(tr => {
      tr.querySelector('[data-act="save"]').addEventListener('click', () => saveRebuildRow(tr));
      tr.querySelector('[data-act="del"]').addEventListener('click', () => deleteRebuildRow(tr));
    });
  }

  async function addRebuildRow() {
    const err = modalEl.querySelector('#stgRbError'); if (err) err.textContent = '';
    const code = (modalEl.querySelector('#stgRbNewCode').value || '').trim();
    const label = (modalEl.querySelector('#stgRbNewLabel').value || '').trim() || null;
    const hours = parseFloat(modalEl.querySelector('#stgRbNewHours').value);
    if (!code) { if (err) err.textContent = 'Enter a transmission code.'; return; }
    if (!Number.isFinite(hours) || hours < 0) { if (err) err.textContent = 'Enter valid book hours.'; return; }
    const { error } = await db.from('rebuild_book_hours')
      .insert({ transmission_code: code, label, book_hours: hours, active: true });
    if (error) { if (err) err.textContent = (error.code === '23505') ? 'That code already exists.' : ('Save failed: ' + error.message); return; }
    renderRebuildHoursSection();
  }
  async function saveRebuildRow(tr) {
    const err = modalEl.querySelector('#stgRbError'); if (err) err.textContent = '';
    const id = tr.dataset.id;
    const label = (tr.querySelector('[data-f="label"]').value || '').trim() || null;
    const hours = parseFloat(tr.querySelector('[data-f="book_hours"]').value);
    const active = tr.querySelector('[data-f="active"]').checked;
    if (!Number.isFinite(hours) || hours < 0) { if (err) err.textContent = 'Enter valid book hours.'; return; }
    const { error } = await db.from('rebuild_book_hours')
      .update({ label, book_hours: hours, active }).eq('id', id);
  ...
  async function deleteRebuildRow(tr) {
    const err = modalEl.querySelector('#stgRbError'); if (err) err.textContent = '';
    const code = (tr.querySelector('td') && tr.querySelector('td').textContent) || 'this rebuild type';
    if (!confirm('Delete rebuild type "' + code + '"? Jobs already saved keep their stored hours.')) return;
    const { error } = await db.from('rebuild_book_hours').delete().eq('id', tr.dataset.id);
