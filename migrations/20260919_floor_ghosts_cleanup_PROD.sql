-- ============================================================================
-- One-time cleanup: cars still on the shop floor whose RO is CLOSED — PROD run
-- (hygemiszxwmyrkmhbjub / www + board). The sandbox has its own file: 20260919_floor_ghosts_cleanup_SANDBOX.sql
...
with listed(po) as (
  ...
  select f.src, f.row_id, f.po, f.row_data, s.statuses
   ...
   where s.statuses = 'closed'                                           -- re-checked NOW
     and (select env from public.app_env limit 1) like 'PROD%'  -- wrong project ⇒ 0 rows
), backed as (
