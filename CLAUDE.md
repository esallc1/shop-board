...
| Call window & Desk (incl. **§6a** what the lanes show, **§6b** key drop box, **§6c** calendar view + same-time chips, **§9** the four outcomes + undo, **§10** the appointment line on the Call Log + customer record) | `call-window-desk.md` | `advisor-board.html` (`callerCard` + `desk` IIFEs), `shared/desk-appointments.js`, `shared/desk-outcomes.js` (+`.test.js`), `shared/call-appointment.js` (+`.test.js`), `api/desk-appointment.js`, `migrations/20260920_calls_outcome_*.sql`, `migrations/20260921_calls_dropoff_key_box_*.sql` |
