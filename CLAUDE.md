...
**Prod code = `bd78d76`** (2026-09-25 — SECURITY SLICE 3 step (a)1, no DB change: the tray CALL CARD writes `calls` only through the new `api/calls.js` (requireUser = active employee; actions `note` / `customer` / `auto_file_ro`, each only its own columns; the server stamps `noted_at` + `noted_by_name` once; the robot only fills an empty `ro_id`; the single-match customer only fills an empty `customer_id`). "Saved ✓" only on a 200; unsaved changes are resent; Close waits. The other 12 browser writers (Desk, Call Log, customer record) still write directly — steps (a)2 / (a)3; the anon + authenticated UPDATE policies stay until the lockdown (b). Cris OK'd after the signed-in check on test.*. Fast-forward `c51dc83..969bdec` (`969bdec` = docs-only on top of `bd78d76`); www/board/apex `/api/version` = `969bdec`, `advisor-board.html` byte-identical to `bd78d76`, `api/calls` 401 unauthenticated; prod read-only: loads, no console errors. `inbox-calls.md` §5, `call-window-desk.md` §1. Previous code ship: `9dce1d4` (in-place recording player). Any later commit on `main` up to the one that wrote this line is docs-only. Update this line on every code ship.
