...
| RO photos (**per-RO** buckets · capture · move · archive) | `ro-photos.md` (§1a = a bucket belongs to ONE RO; §1c = born-with-buckets trigger) | `photo_buckets` (`ro_id` NOT NULL, `archived_by`), `photo_bucket_templates`, `trg_repair_orders_photo_buckets`, `attachments` (`ro_photo`, `bucket_id`, `uploaded_by`, `deleted_at`), `shared/photo-buckets.js` (+`.test.js`), `shared/photo-compress.js`, `my-numbers.html`, `advisor-board.html` (`#view-customer`) |
