...
**Prod code = `8014f95`** (2026-09-24 — WHITEBOARD slices 3 + 4 + the office-whiteboard look: aluminum frame / glossy board / FRONT OFFICE + red date / marker zone titles / handwriting lines with ⚡ auto · ✎ hand / marker tray, fonts self-hosted in `shared/fonts` (Permanent Marker, Kalam). "Called ✓" on Ready lines (who + when, undo; stamp stays with the RO) and Don't forget (+ write on board — box closes after each save — who/when, anyone erases, recently erased 7 days + Undo), all writes via `api/whiteboard.js` (requireUser first, who/when stamped server-side, never writes repair_orders, never DELETE); reads staff-only; realtime on repair_orders + both whiteboard tables after setAuth. Migration `20260924_whiteboard_*` applied SANDBOX + PROD by Cris (8/8 PASS each). Two-person check on test.* (ZZ Test Advisor ↔ ZZ Test Owner) confirmed by Cris. Fast-forward `b713d50..8014f95`; www/board/apex `/api/version` = `8014f95`, all 11 changed served files byte-identical; prod: POST /api/whiteboard no token → 401, anon read of both tables → 42501, Ready list = the 5 real invoice ROs. `whiteboard.md` §2–§8. Previous code ship: `968d376` (Whiteboard slices 1 + 2). Any later commit on `main` up to the one that wrote this line is docs-only. Update this line on every code ship.
...
| Call window & Desk (incl. **§6a** what the lanes show, **§6b** key drop box, **§6c** calendar view + same-time chips, **§6d** row names + the "Who is this?" guess box, **§9** the four outcomes + undo, **§10** the appointment line on the Call Log + customer record) | `call-window-desk.md` | `advisor-board.html` (`callerCard` + `desk` IIFEs), `shared/desk-appointments.js`, `shared/desk-outcomes.js` (+`.test.js`), `shared/call-appointment.js` (+`.test.js`), `shared/desk-names.js` (+`.test.js`), `api/desk-appointment.js`, `migrations/20260920_calls_outcome_*.sql`, `migrations/20260921_calls_dropoff_key_box_*.sql` |
...
| Messenger inbox tray (advisor board) | `messenger-tray.md` (§2 = hidden / open / tucked + auto-open; §3a = actions through `api/messenger.js`; §3b = auto label, typed phone, Attach suggestion) | `shared/messenger-tray.js` (`mountMessengerTray`), `shared/messenger-tray-logic.js` (+`.test.js`), `shared/messenger-tray.css`, `advisor-board.html` (mount before `</body>`); reads `social_threads`/`social_messages` |
| Desk pad (advisor board scratch sticky notes — this computer only) · **the bottom drawer** it lives in (tabs 📝 Desk pad N · 📋 Whiteboard W) | `desk-pad.md` (**§2 = the drawer**: tabs, push-up vs overlay, height, where it sits next to the Facebook tray; §4 = localStorage only, never the DB) | `shared/bottom-drawer.js` (+`.css`, the frame — no DB/network), `shared/front-desk-drawer.js` (`mountFrontDeskDrawer({ db })`, the one mount call), `shared/desk-pad.js` (`createDeskPadPanel`), `shared/desk-pad-logic.js` (+`.test.js`; `isPadToggleKey`/`isBoardToggleKey`), `shared/desk-pad.css`, `advisor-board.html` (mount before `</body>`) |
| Whiteboard (advisor board, shared — the drawer's 2nd tab; **Ready → call for pickup** = `status 'invoice'` + **Called ✓**; **Don't forget** (write / erase / recently erased + Undo); parts zone next) | `whiteboard.md` (**§2 = the look — approved office-whiteboard mockup, self-hosted fonts**; §3 = which ROs + auto-removal by status, never closed_at; §4 = reads, never re-saves an RO; **§7 = tables · §8 = `api/whiteboard.js` — every write, who/when stamped server-side**) | `shared/whiteboard.js` (`createWhiteboardPanel`), `shared/whiteboard-logic.js` (+`.test.js`), `shared/whiteboard.css`, `shared/fonts/` (Permanent Marker + Kalam woff2 + licences), `shared/front-desk-drawer.js`, `api/whiteboard.js` (+`.test.js`), `migrations/20260924_whiteboard_{SANDBOX,PROD}.sql` (+`shared/whiteboard-migration.test.js`); reads `repair_orders`, `whiteboard_items`, `whiteboard_pickup_calls` (+ realtime) |
