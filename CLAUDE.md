...
| Whiteboard (advisor board, shared — the drawer's 2nd tab; **Ready → call for pickup** = `status 'invoice'`, read-only; parts / don't-forget zones next) | `whiteboard.md` (**§2 = the look — approved office-whiteboard mockup, self-hosted fonts**; §3 = which ROs + auto-removal by status, never closed_at; §4 = the one read-only query, never re-saves an RO) | `shared/whiteboard.js` (`createWhiteboardPanel`), `shared/whiteboard-logic.js` (+`.test.js`), `shared/whiteboard.css`, `shared/fonts/` (Permanent Marker + Kalam woff2 + licences), `shared/front-desk-drawer.js`; reads `repair_orders` (+ realtime) |
