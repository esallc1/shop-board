...
**Prod code = `47bdc15`** (2026-09-24 — CALLS INTO THE INBOX TRAY, slice 1 (no DB change): the floating call cards (#callCardStack, z 4000) are gone — every call card lives in the right-hand Inbox tray. Folded strip is the default (48 px full-height, 📞 + f badges, pushes the board; the drawer stops beside it); page load stays folded; the tray opens by itself only for a call ringing now or a newly arrived FB message, and folds back when the last waiting item is handled. Ringing = pinned caller-ID glance (name, phone, vehicle, In shop now, Last visit = newest CLOSED RO (hidden if none), Heads up (hidden if nothing)); "Needs handling" = call rows (no-note on top; can't be closed un-noted) + FB threads; FB threads past 24 h → collapsed "Can't reply anymore" (Call them / Done), not counted. Opened call = the same card in the tray: recording (pending until ready), note, Call back · Coming in · Done (coming), Attach / Start RO / Not a customer (coming). Backfill = today's untouched calls (25). Writes unchanged — direct `calls` updates, temporary until the security slice (Cris's OK). Cris tested on test.* as ZZ Test Advisor. Fast-forward `1682236..47bdc15`; www/board/apex `/api/version` = `47bdc15`, all 12 changed served files byte-identical; prod read-only: folded strip, board padded 48, no floating card. `inbox-calls.md`, `messenger-tray.md` §2, `call-window-desk.md` §2/§2a/§5. Previous code ship: `d170604` (Whiteboard slice 6). Any later commit on `main` up to the one that wrote this line is docs-only. Update this line on every code ship.
