...
**Prod code = `9dce1d4`** (2026-09-25 — RECORDINGS: ▶ Play opens the browser's own `<audio controls>` IN PLACE of the button (play/pause, drag, time) — Call Log, RO Call History, customer record, all via `shared/recording-view.js`; ONE open player board-wide; closing the Call log / another RO / leaving the customer record / any sidebar navigation pauses + collapses it; a player redrawn off the page stops itself; hidden per-place `<audio>` removed; link logic + states unchanged; tray card unchanged. No DB change, no new write. Cris OK'd shipping without a test look (test has no audio files). Fast-forward `0a9539d..6c36e2c` (`6c36e2c` = docs-only on top of `9dce1d4`); www/board/apex `/api/version` = `6c36e2c`, changed code files byte-identical to `9dce1d4`; prod read-only: loads, recording-view.js 200, no console errors. `recordings-audio.md` §3. Previous code ship: `103a645` (Inbox slice 2b). Any later commit on `main` up to the one that wrote this line is docs-only. Update this line on every code ship.
