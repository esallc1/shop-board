...
| Cost & Profit (Cockpit + Build Sheet) | `cost-profit.md` | `shared/build-sheet.js` (cost layer + People & rates), `shared/board-settings.js` (`renderUnitsEditor`+`costLayer`/`wireCostRow`/`renderRebuildUnits`), `unit_parts`, `shop_settings.std_rr_rate`/`rebuilder_cost`/`std_advisor_pct`, `owner-board.html`, `bookkeeping-board.html` (no feature switch) |
