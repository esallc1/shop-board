...
| Desk pad (advisor board scratch sticky notes — this computer only) | `desk-pad.md` (§2 = push-up vs overlay + where it sits next to the Facebook tray; §4 = localStorage only, never the DB) | `shared/desk-pad.js` (`mountDeskPad`), `shared/desk-pad-logic.js` (+`.test.js`), `shared/desk-pad.css`, `advisor-board.html` (mount before `</body>`) |
