...
| Meta / Facebook webhook (receive + verify + **store Messenger messages/echoes**) | `meta-webhook.md` (**§4 = the signature is ENFORCED here, unlike ctm-webhook — do not align the two files**; **§9 = storage into `social_threads`/`social_messages`; §10 = testing on staging with `scripts/meta-sim.mjs`**) | `api/meta-webhook.js` (+`.test.js`, +`meta-webhook-store.test.js`), `migrations/20260923_social_messaging_{SANDBOX,PROD}.sql` (+`shared/social-messaging-migration.test.js`), `public.is_staff()`, `scripts/meta-sim.mjs`; env `META_APP_SECRET`, `META_VERIFY_TOKEN`, `META_PAGE_ACCESS_TOKEN` (unset); no UI yet |
