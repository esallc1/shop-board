...
**Prod code = `d170604`** (2026-09-24 — WHITEBOARD slice 6: 📌 "Pin to whiteboard" on each Desk pad sticky — MOVES the text to Don't forget (server-stamped); the sticky leaves the pad only after the server confirmed, stays with an error otherwise; empty → disabled; > 500 → clear message. desk-pad.js stays network-free (the drawer injects `pinToBoard` → the Whiteboard's `pin`). One 📌 = one line: `createPinner` (one pin per sticky, "pinning…", no auto-retry) + `api/whiteboard.js` duplicate guard (same person + kind + text + RO still on the board within 10 min → the existing line, nothing written) — fixes Cris's one-📌-three-lines bug. No migration. Cris re-tested on test.* as ZZ Test Advisor (rapid 📌 → one line). Fast-forward `9a4fb2d..d170604`; www/board/apex `/api/version` = `d170604`, all 8 changed served files byte-identical; prod: POST /api/whiteboard no token → 401, drawer + 📌 on a sticky (local temp sticky, not pinned, thrown away), Whiteboard = 5 Ready ROs. `whiteboard.md` §6a/§8, `desk-pad.md` §3. Previous code ship: `bc22bc8` (Whiteboard slice 5). Any later commit on `main` up to the one that wrote this line is docs-only. Update this line on every code ship.
