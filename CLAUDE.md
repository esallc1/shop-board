...
**Prod code = `651df27`** (2026-09-25 — INBOX SLICE 2a, no DB change, no new write: ONE mixed "Needs handling" list in the Inbox tray — call rows + Facebook threads merged by `mergeNeedsHandling` (`shared/inbox-list-logic.js`): nobody-answered-yet on top (calls with no note + threads where the customer spoke last; auto-reply / failed send aren't replies), then the rest, newest first by the customer's time (never our reply); FB row time = the customer's last arrival. One draw path, a background redraw waits for keyboard focus; badges / fold / auto-open unchanged, now pure + tested. Cris OK'd on test.*. Fast-forward `242ed60..af98f5c` (`af98f5c` = docs-only on top of `651df27`); www/board/apex `/api/version` = `af98f5c`, all 7 changed code files byte-identical to `651df27`; prod read-only: folded, strip drawn, no console errors. `inbox-calls.md` §3, `messenger-tray.md` §2. Previous code ship: `47bdc15` (calls into the Inbox tray, slice 1). Any later commit on `main` up to the one that wrote this line is docs-only. Update this line on every code ship.
