...
**Prod code = `103a645`** (2026-09-25 — INBOX SLICE 2b, no DB change, no new write: ONE shared call-recording player, `shared/recording-view.js`, for all four places on the advisor board — Call Log, RO Call History, customer record, the tray call card — replacing four copied blocks; one links reader `cdRecordingIndex` (api/recording-links via cdAuthFetch, batches of 50). ▶ Play places look/behave as before; tray card: a call with NO recording row says "shows up after the call ends" for 30 min then "No recording" (was "arrives a few minutes…" forever); a ready row with no signable link → "couldn't be fetched"; one fresh link per card. Cris OK'd on test.* (sandbox has recording rows but no audio files — real playback proven on prod). Fast-forward `d600a5e..1a2cd8a` (`1a2cd8a` = docs-only on top of `103a645`); www/board/apex `/api/version` = `1a2cd8a`, changed code files byte-identical to `103a645`; prod read-only: folded, strip drawn, recording-view.js 200, no console errors. `recordings-audio.md` §3, `inbox-calls.md` §4. Previous code ship: `651df27` (Inbox slice 2a). Any later commit on `main` up to the one that wrote this line is docs-only. Update this line on every code ship.
