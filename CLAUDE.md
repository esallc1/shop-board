...
**Prod code = `968d376`** (2026-09-24 — WHITEBOARD slices 1 + 2 (advisor board): the Desk pad became a two-tab bottom drawer (📝 Desk pad N · 📋 Whiteboard W; one at a time, Esc hides; same push-up/height/tray edge), `mountFrontDeskDrawer({ db })` = one shared module (advisor board only for now). Whiteboard "Ready → call for pickup" = `repair_orders.status = 'invoice'`, one read-only query, realtime on repair_orders + 60 s catch-up, click → `cdOpenRo`; never writes (test-locked). Parts / Don't forget = "Coming next". No prod migration (prod had repair_orders in supabase_realtime since July; sandbox got it 2026-09-24 by Cris — record `migrations/20260924_sandbox_repair_orders_realtime.sql`). Fast-forward `7dbaff0..968d376`; www/board/apex `/api/version` = `968d376`, all 14 changed served files byte-identical; prod read-only check: N/W/Esc work, Whiteboard shows the 5 invoice ROs (#6013 #6065 #6078 #6092 #6098) = a direct query. `whiteboard.md`, `desk-pad.md` §2. Previous code ship: `7fe5e9e` (Facebook after-hours auto-reply). Any later commit on `main` up to the one that wrote this line is docs-only. Update this line on every code ship.
