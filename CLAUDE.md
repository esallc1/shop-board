...
**Prod code = `b9d349c`** (2026-09-25 — SECURITY SLICE 3 step (a)3, no DB change: the CALL LOG + CUSTOMER RECORD writers go through `api/calls.js` — `attach`, `learn_phone` (the phone learn: customers.phone_secondary written by the SERVER only into an empty slot, guard in the write, then learned_phone — one action), `unattach` (link + robot marks off; the learned number cleared only while it's still exactly that number), `not_a_customer` / `clear_not_a_customer`, `file_ro` (only the call's own customer's RO, 409 otherwise). **All 15 browser `calls` writers are now server-side** (a test sweeps every page + shared module). The anon + authenticated UPDATE policies are still open: the lockdown `migrations/20260926_calls_lockdown_{SANDBOX,PROD}.sql` is committed but **NOT RUN** — Cris runs it by hand, sandbox first, after the watch period. Cris OK'd after the signed-in check on test.*. Fast-forward `9f662e6..0979d5e` (`5c15be5` docs, `0979d5e` the migration files); www/board/apex `/api/version` = `0979d5e`, `advisor-board.html` byte-identical to `b9d349c`, migration files 404, unauthenticated `attach` 401; prod read-only: loads, no console errors. `call-window-desk.md` §1/§2c, `call-auto-attach.md`, `customer-record.md`. Previous code ship: `e73cd37` (+ Add no-ring fix). Any later commit on `main` up to the one that wrote this line is docs-only. Update this line on every code ship.
