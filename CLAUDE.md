...
| Whiteboard (advisor board, shared — the drawer's 2nd tab; **Ready → call for pickup** = `status 'invoice'` + **Called ✓**; **Waiting on parts** (optional RO picker, Arrived ✓, recently cleared + Undo); **Don't forget** (write / erase / recently erased + Undo)) | `whiteboard.md` (**§2 = the look — approved office-whiteboard mockup, self-hosted fonts**; §3 = which ROs + auto-removal by status, never closed_at; §4 = reads, never re-saves an RO; **§6b = Waiting on parts**; **§7 = tables · §8 = `api/whiteboard.js` — every write, who/when stamped server-side**) | `shared/whiteboard.js` (`createWhiteboardPanel`), `shared/whiteboard-logic.js` (+`.test.js`), `shared/whiteboard.css`, `shared/fonts/` (Permanent Marker + Kalam woff2 + licences), `shared/front-desk-drawer.js`, `api/whiteboard.js` (+`.test.js`), `migrations/20260924_whiteboard_{SANDBOX,PROD}.sql` (+`shared/whiteboard-migration.test.js`); reads `repair_orders`, `whiteboard_items`, `whiteboard_pickup_calls` (+ realtime) |
