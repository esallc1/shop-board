...
**Prod code = `bc22bc8`** (2026-09-24 — WHITEBOARD slice 5: Waiting on parts — "+ write on board" with an optional RO picker (open ROs by number / customer / vehicle) + note, line "#6089 · Ford F-250 · SMITH — note — who · when" (RO part opens the RO), Arrived ✓ / × → "recently cleared" (arrived / erased) + Undo, box closes after save; a parts line whose RO is closed leaves the board by itself — derived at read time from the RO's status, listed as "RO closed" (no time: closed_at is the set-once pay stamp; no Undo; reopen brings it back), no write. `api/whiteboard.js`: add with a closed RO → 409. No migration. Cris tested on test.* as ZZ Test Advisor. Fast-forward `95131a4..bc22bc8`; www/board/apex `/api/version` = `bc22bc8`, all changed served files byte-identical; prod: POST /api/whiteboard no token → 401, parts zone + picker loaded, Ready = the 5 real ROs (pane not signed in, so the write link wasn't eyeballed on prod). `whiteboard.md` §6b. Previous code ship: `8014f95` (Whiteboard look + slices 3 + 4). Any later commit on `main` up to the one that wrote this line is docs-only. Update this line on every code ship.
