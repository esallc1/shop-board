...
| Call window & Desk (incl. **§6a** what the lanes show, **§9** the four outcomes + undo, **§10** the appointment line on the Call Log + customer record) | `call-window-desk.md` | `advisor-board.html` (`callerCard` + `desk` IIFEs), `shared/desk-appointments.js`, `shared/desk-outcomes.js` (+`.test.js`), `shared/call-appointment.js` (+`.test.js`), `migrations/20260920_calls_outcome_*.sql` |
