...
**Prod code = `49cd111`** (2026-09-23 — Messenger step 3: `api/messenger.js` reply / link / done,
staff only via `requireUser`; replies answer 503 "not connected" on prod until
`META_PAGE_ACCESS_TOKEN` exists; `META_SEND_MODE=dry-run` on Preview·staging only. Also `CLAUDE.md`
now in `.vercelignore` — `/CLAUDE.md` is 404 on www/board/apex. Fast-forward `1bc5b04..49cd111`.
`meta-webhook.md` §11. Previous code ship: `cf12564` (webhook storage). Any later commit on `main` up to the one that wrote this line is docs-only. Update this line on every code ship.
...
| Meta / Facebook webhook (receive + verify + **store Messenger messages/echoes**) · **Messenger tray actions** (reply / link / done) | `meta-webhook.md` (**§4 = the signature is ENFORCED here, unlike ctm-webhook — do not align the two files**; **§9 = storage into `social_threads`/`social_messages`; §10 = testing on staging with `scripts/meta-sim.mjs`**) | `api/meta-webhook.js` (+`.test.js`, +`meta-webhook-store.test.js`), `api/messenger.js` (+`.test.js`), `migrations/20260923_social_messaging_{SANDBOX,PROD}.sql` (+`shared/social-messaging-migration.test.js`), `public.is_staff()`, `scripts/meta-sim.mjs`; env `META_APP_SECRET`, `META_VERIFY_TOKEN`, `META_PAGE_ACCESS_TOKEN` (unset), `META_SEND_MODE` (`dry-run` on Preview·staging ONLY); no UI yet |
| Messenger inbox tray (advisor board, read-only for now) | `messenger-tray.md` (§2 = hidden / open / tucked + auto-open; §4 = signed-in session only, never writes) | `shared/messenger-tray.js` (`mountMessengerTray`), `shared/messenger-tray-logic.js` (+`.test.js`), `shared/messenger-tray.css`, `advisor-board.html` (mount before `</body>`); reads `social_threads`/`social_messages` |
