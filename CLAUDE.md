...
**Prod code = `254dc9c`** (2026-09-25 — SECURITY SLICE 3 step (a)2, no DB change: the DESK's six writers go through `api/calls.js` — `outcome` (Arrived / Not coming / Called clear, only an open row, 409 otherwise; Follow up moves to Callbacks, `outcome_prev_due_at` from the row), `undo`, `done`, `edit`, `reschedule` (a timed slot clears the key box in the same write); patches built on the server with the same shared rules (`outcomePatch`, `undoPatch`, `withKeyBox`, `reschedulePatch`), who/when from the signed-in employee; failures shown, never faked. With (a)1 (`bd78d76`, the call card) that is 9 of 15 browser `calls` writers moved; the Call Log / customer record writers (#4–9) are step (a)3; the anon + authenticated UPDATE policies stay until the lockdown (b). Cris OK'd after the signed-in Desk check on test.* (+ his own Follow up by hand). Fast-forward `8b8f8af..44fca0e` (`44fca0e` = docs-only on top of `254dc9c`); www/board/apex `/api/version` = `44fca0e`, `advisor-board.html` byte-identical to `254dc9c`, unauthenticated Desk action 401; prod read-only: loads, no console errors. `call-window-desk.md` §1/§7/§9d. Previous code ship: `bd78d76` (slice 3 (a)1). Any later commit on `main` up to the one that wrote this line is docs-only. Update this line on every code ship.
