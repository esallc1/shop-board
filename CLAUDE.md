...
| RO job category (Transmission rebuild / General repair — RO ONLY, never the floor rows) | `ro-checkin-tech.md` (**§7**; the floor rows' old `job_category` tag is separate — `tech-board.md` §4) | `shared/job-category.js` (+`.test.js`, THE list), `repair_orders.job_category` (+ CHECK), `advisor-board.html` (`#cdRoJobCategory`, `renderJobCategory`/`setJobCategory`, `archiveToCompletedJobs` → `completed_jobs.job_category`), `migrations/20260919_ro_job_category_{SANDBOX,PROD}.sql` |
